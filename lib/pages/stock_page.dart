...
class _StockPageState extends State<StockPage> {
  ...
  DateTime lastMonth = DateTime(DateTime.now().year, DateTime.now().month - 1, DateTime.now().day);
  ...
  void initState() {
    ...
    predictedPrice = ApiService().fetchStockDataToday(widget.stockCode);
            ...
            Padding(
              ...
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: FutureBuilder<List<StockData>>(
                      future: ApiService().fetchStockDataTimed(widget.stockCode,lastMonth,yesterday),
                      builder: (BuildContext context, AsyncSnapshot<List<StockData>> snapshot) {
                        if (snapshot.hasData) {
                          // Get the most recent closing price from the fetched data
                          double yesterdayPrice = snapshot.data!.last.closingPrice;
                          double lastMonthPrice = snapshot.data!.first.closingPrice;
                          double percentageChange = (yesterdayPrice- lastMonthPrice) * 100 / lastMonthPrice;
                          // Update the mostRecentPrice variable
                          var mostRecentPrice = percentageChange.toStringAsFixed(2);
                          return Text(
                            'Change from Last Month: ${percentageChange.toStringAsFixed(2)}%',
                            style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: percentageChange > 0 ? Colors.green : Colors.red),
                          );
                        } else if (snapshot.hasError) {
                          return Text("${snapshot.error}");
                        } else {
